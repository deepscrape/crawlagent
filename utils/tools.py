...
async def remove_stale_clients(now) -> None:
    ...
    async with active_connections_lock:
        ...
        for conn_id, conn in websocket_manager.active_connections.items():
...
def get_wsl2_host_ip():
    """Get the appropriate host IP address for WebRTC connections.
    
    For WSL2, gets the host Windows IP for external connections.
    For Windows, returns the first non-loopback IPv4 address.
    For Linux, returns the default route gateway.
    """
    try:
        is_wsl = "microsoft-standard" in platform.uname().release.lower() or \
                 os.path.exists("/proc/sys/fs/binfmt_misc/WSLInterop")
        
        if is_wsl:
            # Get the WSL2 host Windows IP address
            try:
                # First attempt: check /etc/resolv.conf
                with open("/etc/resolv.conf") as f:
                    for line in f:
                        if line.startswith("nameserver"):
                            return line.split()[1]
                
                # Second attempt: use ip route
                route = subprocess.check_output("ip route | grep default", shell=True).decode()
                return route.split()[2]
            except Exception as e:
                logger.warning(f"Failed to get WSL2 host IP: {e}")
                return "172.17.0.1"  # Common default for WSL2
            ...
            for ip in ip_list:
                if ip[0] == socket.AF_INET and not str(ip[4][0]).startswith('127.'):
                    return ip[4][0]
            return "127.0.0.1"
        else:
            # Regular Linux
        ...
        logger.warning(f"Error determining host IP: {e}")
        return "127.0.0.1"
